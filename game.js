...
// --- Variables de Sonido, Assets y Cheats ---
    ...
    class Missile { constructor(x, y, target) { this.image = assets.missile; this.x = x; this.y = y; this.target = target; this.width = 60 * scaleFactor; this.height = 60 * scaleFactor; this.speed = 4 * scaleFactor; this.damage = 15; } draw() { if (this.image) { ctx.save(); ctx.translate(this.x + this.width / 2, this.y + this.height / 2); const angle = this.target ? Math.atan2(this.target.y - this.y, this.target.x - this.x) + Math.PI / 2 : 0; ctx.rotate(angle); ctx.drawImage(this.image, -this.width / 2, -this.height / 2, this.width, this.height); ctx.restore(); } } update() { if (this.target && this.target.health > 0) { const angle = Math.atan2(this.target.y + this.target.height / 2 - this.y, this.target.x + this.target.width / 2 - this.x); this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; } else { this.y -= this.speed; } } }
...
function triggerComboIndicator(x, y, tier) {
    ...
    function addScore(basePoints) {
        if (scoreAndStatsDisabled) return;
        const diff = DIFFICULTY_SETTINGS[difficultyLevel];
    ...
    function handleTargetDestroyed(target) {
        if (target instanceof Boss) {
            ...
            addScore(scoreForBoss); // CAMBIO 13: Usar nuevo sistema de puntuación
            ...
            intermissionData = { name: BOSS_NAMES[target.bossType] || BOSS_NAMES['REGULAR'], score: Math.ceil(scoreForBoss * (DIFFICULTY_SETTINGS[difficultyLevel].scoreMultiplier)), startTime: Date.now() };
            gameState = 'INTERMISSION';
            playSound(audioAssets.intermission);
            triggerScreenFlash(500);

        } else if (target instanceof Asteroid) {
            addScore(target.health); // CAMBIO 13: Asteroides dan puntos normales
            ...
            addScore(target.health); // CAMBIO 13: Usar nuevo sistema de puntuación
